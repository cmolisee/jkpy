...
class Normalize(Handler):
    ...
    def process(self, model: MenuModel, view: MenuView) -> None:
        ...
        for issue in model.data["raw_issues"]:
            ...
            row["primary_developer"] = (
                None
                if not fields.get("customfield_10264", {})
                else fields.get("customfield_10264", {}).get("displayName", None)
            )

            row["team"] = (
                None
                if not fields.get("customfield_10235", {})
                else fields.get("customfield_10235", {}).get("value", None)
            )
    ...
    def get_valid_done_date(self, model: MenuModel, changelog: Any) -> None | datetime:
        ...
        for t in reversed(transitions):
            if t["to_category"].lower() in [
                s.lower().replace('"', "") for s in model.data["statuses"]
            ]:
