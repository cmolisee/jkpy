...
class Validate(Handler):
    ...
    def process(self, model: MenuModel, view: MenuView) -> None:
        ...
        null_indices = (
            ...
            .with_row_index()
            .filter(pl.col("primary_developer").is_null())["index"]
            .to_list()
        )
        time.sleep(1.5)
        ...
        for idx in null_indices:
            choice = self.prompt_action(
                model.data["data_frames"]["normalized"].row(idx, named=True)
            )
            pd_updates[idx] = choice

        if len(pd_updates) > 0:
            s = model.data["data_frames"]["normalized"]["primary_developer"].clone()
            model.data["data_frames"]["normalized"] = model.data["data_frames"][
                "normalized"
            ].with_columns(
                pl.Series(
                    "primary_developer",
                    s.scatter(list(pd_updates.keys()), list(pd_updates.values())),
                )
            )
    ...
    def jira_update(self, key: str, choice: str) -> None:
        ...
        with httpx.Client(verify=False) as client:
            response = client.put(
                ...
                content=json.dumps(
                    {"fields": {"customfield_10264": {"accountId": account["accountId"]}}}
                ),
            )
        ...
        if response.status_code != 204 and response.status_code != 200:
            print(
                Ansi.YELLOW
                + f">>> Updating primary developer for {key} was NOT successful"
                + Ansi.RESET
            )
        else:
            print(f">>> Updated {key} with {choice}")

    def prompt_action(self, row: dict[Any, Any]) -> str | None:
