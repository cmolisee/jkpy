...
class Metrics(Handler):
    ...
    def process(self, model: MenuModel, view: MenuView) -> None:
        ...
        df_per_dev = (
            model.data["data_frames"]["exploded"]
            ...
            .with_columns(
                pl.col("time_tracking").map_elements(
                    self.format_time_tracking, return_dtype=pl.String
                )
            )
        ...
        df_per_primary_dev = (
            model.data["data_frames"]["filtered"]
        ...
        df_result = df_per_dev.join(
            ...
            on="developer",
            how="left",
        ).with_columns(pl.col("no_tracking_deficit").fill_null(0.000).cast(pl.Float64))
    ...
    def other_labels(self, model: MenuModel) -> Iterable[IntoExpr]:
        return [pl.col(f"is_{label}").sum().alias(label) for label in model.data["labels"]]

    def format_time_tracking(self, s: str) -> str:
    ...
    def tracking_deficit(self) -> IntoExpr:
        return (
            (
                pl.col("time_tracking")
                .filter(pl.col("time_tracking").is_null() | (pl.col("time_tracking") == 0))
                .count()
                / pl.len()
                * 100
            )
            .round(3)
            .alias("no_tracking_deficit")
        )
